...
from ...database.models import Project, User, AdminUser, ProjectFile
...
async def get_projects(
    ...
    try:
        ...
        for project in projects:
            ...
            # Добавляем информацию о Timeweb если есть
            if project.project_metadata and 'timeweb_credentials' in project.project_metadata:
    ...
    try:
        ...
        if user and user.telegram_id:
            try:
                ...
                # Уведомление клиенту
                status_name = PROJECT_STATUSES.get(new_status, new_status)
                message = f"📋 Статус вашего проекта '{project.title}' изменен на: {status_name}"
                if comment:
                    message += f"\n\n💬 Комментарий: {comment}"
        ...
        return {
            "success": True,
            "message": f"Статус успешно обновлен на '{PROJECT_STATUSES[new_status]}'" + 
